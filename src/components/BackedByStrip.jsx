      ...
      <ul className="space-y-3">
        {backers.map((b) => (
          <li key={b.name} className="flex items-center gap-3">
            <div className="h-10 w-14 bg-white rounded-md flex items-center justify-center p-1 shrink-0">
              <img src={b.src} alt={`${b.name} logo`} className="max-h-8 max-w-full object-contain" />
            </div>
            <div>
              <div className="font-medium text-ink text-sm">{b.name}</div>
              <div className="text-inkSoft/70 text-xs">{b.note}</div>
            </div>
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="grid grid-cols-3 gap-4 md:gap-6 items-stretch">
      {backers.map((b) => (
        <div
          key={b.name}
          className="bg-paper border border-line rounded-2xl px-3 md:px-6 py-5 md:py-7 flex flex-col items-center justify-center text-center gap-3 min-h-[140px] transition-colors hover:border-terra/40"
        >
          ...
          <div>
            <div className="font-serif text-sm md:text-base font-semibold text-ink">{b.name}</div>
